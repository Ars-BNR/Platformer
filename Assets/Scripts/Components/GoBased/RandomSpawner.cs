...
namespace Platformer.Components.GoBased
{
    public class RandomSpawner : MonoBehaviour
    {
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            var middleAngeDelta = (180 - _sectorRotation - _sectorAngle) / 2;
            var rightBound = GetUnitOnCircle(middleAngeDelta);
            Handles.DrawLine(position, position + rightBound);


            var leftBound = GetUnitOnCircle(middleAngeDelta + _sectorAngle);
