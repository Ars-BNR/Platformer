...
namespace Assets.Effects
{
    public class InfiniteBackground : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            {
                InstantiateConatiner(_allBounds.min.x - _containerBounds.extents.x);
            }
            if (!_allBounds.Contains(screenRight))
            {
                InstantiateConatiner(_allBounds.max.x + _containerBounds.extents.x);
            }
        }

        private void InstantiateConatiner(float boundCenterX)
        {
